...
def hits(dec_range, ra_range, res=10.0, taper=4.0, holes=0, seed=0):
    ...
    box = (
        np.array([[dec_range[0] - taper, ra_range[0]], [dec_range[1] + taper, ra_range[1]]])
        * utils.degree
    )
    shape, wcs = enmap.geometry(pos=box, res=res * utils.arcmin, proj="car")
    dec, ra = enmap.posmap(shape, wcs) / utils.degree

    nhit = np.clip((dec - dec_range[0] + taper) / taper, 0, 1) * np.clip(
        (dec_range[1] + taper - dec) / taper, 0, 1
    )
    rng = np.random.default_rng(seed)
    for _ in range(holes):
        d0, r0 = rng.uniform(*dec_range), rng.uniform(*ra_range)
        nhit[np.hypot(dec - d0, (ra - r0) * np.cos(np.radians(d0))) < rng.uniform(1.5, 4.0)] = 0
...
def cap(centre, radius, res=12.0):
    ...
    box = np.array([[dec0 - radius - 3, ra0 - pad], [dec0 + radius + 3, ra0 + pad]]) * utils.degree
    shape, wcs = enmap.geometry(pos=box, res=res * utils.arcmin, proj="car")
...
if __name__ == "__main__":
    ...
    areas = plot_footprints(
        maps, output=out, size=560, title="Survey footprints", galactic_plane=True
    )
