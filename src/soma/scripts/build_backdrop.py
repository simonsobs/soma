...
if __name__ == "__main__":
    ...
    ap.add_argument("-r", "--res", type=float, default=0.5,
                    help="output resolution in degrees (default: 0.5)")
    ap.add_argument("-f", "--field", type=int, default=0,
                    help="column of the HEALPix file to read (default: 0)")
    a = ap.parse_args()

    omap, nside, lmax = make_backdrop(a.input, res=a.res, field=a.field)
    enmap.write_map(a.output, omap)
    print(f"nside {nside} -> {omap.shape[0]}x{omap.shape[1]} CAR at {a.res} deg "
