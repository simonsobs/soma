...
def build_parser():
    ...
    add("-o", "--output", default="footprints.gif",
        help=".gif for the animation, .png/.pdf/.svg for a still")
    add("-l", "--labels", help="comma-separated names, one per map "
                               "(default: the file names)")
    add("-c", "--colors", help="comma-separated matplotlib colours, one per map")
    add("-t", "--threshold", type=float, default=0.0,
        help="a pixel is observed where ivar > this")
    add("--title", help="figure title")

    g = p.add_argument_group("animation")
    g.add_argument("--nframes", type=int, default=180,
                   help="frames per turn; 1 gives a still")
    g.add_argument("--fps", type=int, default=15, help="playback rate")
    g.add_argument("--ra0", type=float, default=0.0,
                   help="RA at the centre of the first frame, degrees")
    g.add_argument("--dec0", type=float, default=-20.0,
                   help="declination the view is centred on, degrees")
    g.add_argument("--wobble", type=float, default=20.0,
    ...
    g.add_argument("--res", type=float, default=0.2,
                   help="internal all-sky grid resolution, degrees")
    g.add_argument("--alpha", type=float, default=0.7, help="opacity of the fills")
    g.add_argument("--grid", type=float, default=30.0,
                   help="graticule spacing in degrees; 0 disables it")
    g.add_argument("--ra-units", choices=("hours", "deg"), default="hours",
                   help="units for the RA labels")
    g.add_argument("--ra-sign", type=int, choices=(-1, 1), default=-1,
                   help="-1 puts RA increasing to the left, as seen from Earth")
    g.add_argument("--galactic-plane", action="store_true",
                   help="overlay the Galactic plane")
    g.add_argument("--backdrop", default="auto",
...
def main(argv=None):
    ...
    areas = plot_footprints(
        a.ivars, labels=split(a.labels), output=a.output, threshold=a.threshold,
        colors=split(a.colors), nframes=a.nframes, fps=a.fps, dec0=a.dec0,
        wobble=a.wobble, ra0=a.ra0, grid=a.grid, ra_units=a.ra_units, size=a.size,
        res=a.res, galactic_plane=a.galactic_plane, title=a.title,
        ra_sign=a.ra_sign, alpha=a.alpha,
