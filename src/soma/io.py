...
def plot_footprints(ivars, labels=None, output="footprints.gif", *, threshold=0.0,
                    colors=None, nframes=180, fps=15, dec0=-20.0, wobble=20.0, ra0=0.0,
                    grid=30.0, ra_units="hours", size=720, res=0.2,
                    galactic_plane=False, title=None, ra_sign=-1, backdrop="auto",
                    alpha=0.7):
    ...
    if len(items) > 3:
        warnings.warn(f"{len(items)} footprints on one globe: past three, colours "
                      "alone stop separating regions that touch; names are "
                      "written on the footprints too", RuntimeWarning, stacklevel=2)
    ...
    row_area = np.asarray(enmap.pixsizemap(gshape, gwcs, separable=True))[:, :1] \
        / utils.degree ** 2
    ...
    for label, src in items:
        ...
        if not m.any():
            warnings.warn(f"footprint {label!r} is empty at threshold {threshold}",
    ...
    shade = (_AMBIENT + (1 - _AMBIENT)
             * np.clip(x * light[0] + y * light[1] + z * light[2],
                       0, 1) ** 0.75).astype(np.float32)
    palette = np.array([to_rgb(GLOBE)] + [to_rgb(c) for c in colors[:len(items)]],
                       dtype=np.float32)

    # --- figure
    still = nframes <= 1 or (output and os.path.splitext(output)[1].lower() != ".gif")
    fs = max(7.0, size / 78)
    legend, lcols, lrows = _legend(items, areas, colors, galactic_plane, alpha,
                                   size / 100, fs)
    fig = Figure(figsize=(size / 100, (size + (46 if title else 10)) / 100 + 0.34 * lrows),
                 dpi=100, facecolor=SURFACE)
    FigureCanvasAgg(fig)
    h = fig.get_figheight()
    ax = fig.add_axes([0, 0.34 * lrows / h, 1, size / 100 / h])
    if title:
        fig.text(0.5, 1 - 0.23 / h, title, ha="center", va="center", color=INK,
                 fontsize=max(11, size / 52))
    if lrows:
        fig.legend(handles=legend, loc="lower center", frameon=False, fontsize=fs,
                   ncol=lcols, labelcolor=INK,
                   handlelength=1.4, handleheight=0.9, borderaxespad=0.35)
    ...
    for view, tilt in zip(views, tilts, strict=True):
        ...
        for k in order:
            hit = masks[k][iy + ix]
            ids[idx_in[hit]] = k
        _draw(ax, ids.reshape(S, S), palette, shade, disc, ss, size, view, tilt,
              ra_sign, grid, ra_units, fs, regions, galactic_plane, alpha=alpha,
              tex=None if sky is None else sky[iy + ix], idx_in=idx_in)
        if not still:
            fig.canvas.draw()
...
def _sources(ivars, labels):
    ...
    for i, (label, src) in enumerate(items):
        if label is None:
            label = (os.path.basename(os.fspath(src)).split(".")[0]
                     if isinstance(src, (str, os.PathLike)) else f"map {i + 1}")
...
def _backdrop(src, gshape, gwcs):
    ...
    if isinstance(src, str) and src == "auto":
        ...
        if not src or not os.path.exists(src):
            return None
    imap = enmap.read_map(src) if isinstance(src, (str, os.PathLike)) else src
    imap = enmap.enmap(np.asarray(imap, dtype=np.float32).reshape(
        (-1,) + imap.shape[-2:])[0], imap.wcs, copy=False)
    ...
    norm = ImageNormalize(v, interval=AsymmetricPercentileInterval(5, 99.5),
                          stretch=PowerStretch(BACKDROP_GAMMA), clip=True)
...
def _label_anchor(mask, gshape, gwcs):
    ...
    return (float(cv[0] + (xs[j] + 1 - cp[0]) * cd[0]),
            float(cv[1] + (ys[j] + 1 - cp[1]) * cd[1]))
...
def _forward(ra, dec, ra0, dec0, ra_sign):
    ...
    return (ra_sign * np.cos(d) * np.sin(dl),
            np.cos(d0) * np.sin(d) - np.sin(d0) * np.cos(d) * np.cos(dl),
            np.sin(d0) * np.sin(d) + np.cos(d0) * np.cos(d) * np.cos(dl))


def _draw(ax, ids, palette, shade, disc, ss, size, view, dec0, ra_sign, grid,
          ra_units, fs, regions, galactic_plane, alpha=1.0, tex=None,
          idx_in=None):
    ax.clear()
    ...
    if len(regions) > 1:
        try:                    # a thin gap, not an outline, where two fills meet
            ...
            w = 2 * ss + 1
            gapped = np.where(ndimage.maximum_filter(ids, w, mode="nearest")
                              != ndimage.minimum_filter(ids, w, mode="nearest"), -1, ids)
        except ImportError:                                  # pragma: no cover
            pass
    ...
    rgb[covered] = (alpha * palette[gapped[covered] + 1]                # sky reads
                    + (1 - alpha) * rgb[covered])                      # through them
    rgb = (rgb * shade[..., None]).reshape(size, ss, size, ss, 3)
    ax.imshow(np.dstack([rgb.mean((1, 3)), disc]), extent=(-1, 1, -1, 1),
              origin="lower", interpolation="antialiased", zorder=1)
    ...
    if galactic_plane:
        ...
        _curve(ax, eq[0], eq[1], view, dec0, ra_sign, color=MUTED, lw=1.2,
               ls=(0, (5, 3)), alpha=0.75, zorder=5)
    ...
    for k, (label, col, anchor) in enumerate(regions):
        ...
        if not _text_fits(ids[::ss, ::ss] == k, sx, sy, label, fs, ax.figure.dpi):
            continue
        ax.text(sx, sy, label, color=_ink_on(col), fontsize=fs, ha="center",
                va="center", zorder=7,
                path_effects=[patheffects.withStroke(linewidth=1.8, foreground=col)])


def _curve(ax, ra, dec, view, dec0, ra_sign, color=MUTED, lw=0.6, ls="-",
           alpha=0.30, zorder=3):
    x, y, near = _forward(ra, dec, view, dec0, ra_sign)
    hidden = near <= 0
    ax.plot(np.where(hidden, np.nan, x), np.where(hidden, np.nan, y), color=color,
            lw=lw, ls=ls, alpha=alpha, zorder=zorder, solid_capstyle="round")
...
def _grid_labels(ax, view, dec0, ra_sign, grid, ra_units, fs, stroke):
    ...
    for ra, xi, yi, ni in zip(ras, x, y, near, strict=True):
        ...
        ax.text(xi, yi + 0.035, text, color=MUTED, fontsize=fs, ha="center",
                va="bottom", zorder=6, path_effects=stroke)
    for dec in np.arange(-90 + grid, 89.99, grid):
        ...
        if not np.isfinite(cos_dl) or abs(cos_dl) > 1:
            ...
            if near > 0:
                ax.text(xi, yi, f"{dec:+g}°".replace("+0", "0"), color=MUTED,
                        fontsize=fs, ha="center", va="bottom", zorder=6,
                        path_effects=stroke)
            continue
        ...
        for s in (1, -1):                       # take the crossing on the left
            ...
            if xi <= 0:
                ...
                ax.text(1.07 * xi / r, 1.07 * yi / r, f"{dec:+g}°".replace("+0", "0"),
                        color=MUTED, fontsize=fs, ha="right", va="center", zorder=6)
                break
...
def _legend(items, areas, colors, galactic_plane, alpha, width_in, fs):
    ...
    handles = [Patch(facecolor=tuple(alpha * np.array(to_rgb(c)) + (1 - alpha) * sky),
                     edgecolor="none", label=f"{lab} — {areas[lab]:,.0f} deg²")
               for (lab, _), c in zip(items, colors, strict=False)]  # may over-supply
    if galactic_plane:
        handles.append(Line2D([0], [0], color=MUTED, lw=1.2, ls=(0, (5, 3)),
...
def _write_gif(frames, path, fps, keep=()):
    ...
    ramp = [tuple(int(round(255 * v * s)) for v in to_rgb(c))
            for c in keep for s in _SHADE_STEPS]
    base = Image.fromarray(frames[0]).quantize(colors=256 - len(ramp),
                                               method=Image.Quantize.MEDIANCUT)
    pal = base.getpalette()[:3 * (256 - len(ramp))] + [v for c in ramp for v in c]
    base.putpalette(pal)
    q = [Image.fromarray(f).quantize(palette=base, dither=Image.Dither.NONE)
         for f in frames]
    ...
    q[0].save(path, save_all=True, append_images=q[1:], loop=0, disposal=2,
              duration=max(20, 10 * int(round(100 / fps))), optimize=True)
